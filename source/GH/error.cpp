    ...
    void triggerError(errors error, const std::string& data) {
        switch(error) {
            ...
            case errors::RESOURCE_ERROR:
                ...
                break;
            case errors::CORE_LUA_ERROR:
                ::userLogger.get()->error("Core lua code errored with message \"{}\", exiting with code {}", data, (int)error);
                exit((int)error);
                break;
            case errors::LUA_ERROR:
                ::userLogger.get()->error("Lua code errored with message \"{}\"", data);
                break;
        }
    }
}
